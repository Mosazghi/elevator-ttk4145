...
func setupTestElevator(t *testing.T) ElevatorState {
	...
	return NewElevator(BehaviorIdle, elevio.MDUp, elevIoDriver)
}
...
func TestSetAction(t *testing.T) {
	...
	// CASE 1: Set action
	action1 := MoveAction{BehaviorMoving, elevio.MDUp}
	e.DoMotorAction(action1)
	assert.Equal(t, BehaviorMoving, e.Behavior, "Elevator should be set to moving")
	assert.Equal(t, elevio.MDUp, e.Dir, "Elevator should be set motor direction up")

	// CASE 2: Set illegal action
	prevState := e
	action2 := MoveAction{BehaviorMoving, 100}
